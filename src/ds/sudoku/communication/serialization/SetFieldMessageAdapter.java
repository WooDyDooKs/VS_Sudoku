...
public class SetFieldMessageAdapter extends MessageSerializer implements
	...
	public SetFieldMessage deserialize(JsonElement jsonMessage, Type type,
		...
		//    Extract index
		JsonElement indexElement = jsonMessageObject.get(SerializationKeys.INDEX_KEY);
		final int index = indexElement.getAsInt();
		
		//    Extract sudoku width and height
		JsonElement widthElement = jsonMessageObject.get(SerializationKeys.SUDOKU_WIDTH_KEY);
		JsonElement heightElement = jsonMessageObject.get(SerializationKeys.SUDOKU_HEIGHT_KEY);
		...
		//    Extract value
		JsonElement valueElement = jsonMessageObject.get(SerializationKeys.VALUE_KEY);
